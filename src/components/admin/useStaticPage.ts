...
export function useStaticPage<T>(slug: string, title: string, merge: (saved: Partial<T> | null) => T): StaticPageState<T> {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (cancelled) return
        if (res.ok) {
          const d = await res.json() as { page: { content_json: Partial<T> | null; status: Status; seo_title: string | null; seo_description: string | null; seo_json?: SeoJson | null } }
          if (cancelled) return
  ...
  const save = useCallback(async (nextStatus?: Status) => {
    ...
    try {
      const res = await fetch(`/api/admin/pages/static/${slug}`, {
        ...
        body: JSON.stringify({ title, content_json: content, seo_title: seoTitle || null, seo_description: seoDesc || null, status: nextStatus ?? status }),
      })
  ...
  }, [slug, title, content, seoTitle, seoDesc, status])
  ...
  return { loading, saving, savedLabel, content, setContent, patch, status, setStatus, seoTitle, setSeoTitle, seoDesc, setSeoDesc, seoJson, setSeoJson, saveSeo, save }
}
